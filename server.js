...
app.post('/add-topic', async (req, res) => {
    try {
        ...
        try {
            ...
            if (newTopic) {
                const updatedChapter = await Chapter.findByIdAndUpdate(chapterId, { $push: { topics: { name: name, topicId: newTopic._id } } }, // Push the new video link to the videos array
...
app.patch('/add-video-content', async (req, res) => {
    try {
        ...
        const updatedChapter = await Chapter.findByIdAndUpdate(chapterId, { $push: { videos: videoLink } }, // Push the new video link to the videos array
        { new: true } // Return the updated document
        );
        if (!updatedChapter) {
...
app.patch('/update-topic-description', async (req, res) => {
    try {
        ...
        const updatedTopic = await Topic.findByIdAndUpdate(topicId, { $set: { description: description } }, { new: true } // Return the updated document
        );
        if (!updatedTopic) {
            return res.status(404).send('Chapter not found');
        }
